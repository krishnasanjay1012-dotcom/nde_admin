...
import AdminPasswordDrawer from "../../pages/User-Password-Rest/user-password-reset";
...
const CustomerList = () => {
  ...
  const [adminPasswordDrawerOpen, setAdminPasswordDrawerOpen] = useState(false);
  ...
  const { data, isLoading } = useClientCustomView({
    page,
    limit,
    filter: filterType,
  ...
  const toggleSelectAll = () => {
    if (selectedIds?.length === customers?.length) {
      ...
      setSelectedIds(customers.map((row) => row._id));
    }
  };
  ...
  const handlePasswordChange = (row) => {
    setPassword(row);
    setAdminPasswordDrawerOpen(true);
  };

  const confirmDelete = () => {
    if (deleteTarget) {
      deleteClientMutation.mutate(deleteTarget._id, {
        onSuccess: () => {
  ...
  const handleDeleteFilter = (item) => {
    removeView(
      { module: "customer", viewID: item.id },
    );
  };
  ...
  const columns = useMemo(() => {
    ...
    const mappedColumns = headers
      ...
      .map((col) => ({
        ...
        header: col.accessorKey === "first_name" ? "Customer Name" : col.header,
        cell: ({ row }) => {
        ...
        <Box flex={1} overflow="hidden">
          ...
          {selectedIds?.length > 0 ? (
            <ActionBar
              ...
              actions={[
                ...
                { label: "Associate To New User", onClick: handleAssociateClick },
              ]}
            ...
            >
              ...
              <Box display="flex" alignItems="center" gap={1}>
                <CommonButton onClick={handleNew} label="New Customer" />
                <MoreActionsMenu
                  ...
                  onChange={(data) => {
                    ...
                    if (data.type === "child" && data.parent === "Import" && data.label === "Import Customers") {
                      navigate('import');
                    }
                  }}
                />
          ...
          <ReusableTable
            ...
            onRowClick={(row) => {
              ...
              navigate(`/customers/details/${row._id}?${queryString}`);
            }}
            isLoading={isLoading}
            sortableColumns={
              headers
              ...
              <RowActions
                ...
                actions={[
                  ...
                  },
                  {
                    ...
                    icon: (
                      <PasswordIcon sx={{ color: "blue", fontSize: 20 }} />
                    ),
          ...
          <AdminPasswordDrawer
            open={adminPasswordDrawerOpen}
            handleClose={() => setAdminPasswordDrawerOpen(false)}
            isEdit={false}
