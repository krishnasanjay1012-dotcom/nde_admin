...
import { useCreatePlan, useUpdatePlan } from '../../../hooks/application/application-hooks';
...
const mapInitialData = (data) => {
  ...
  return {
    ...
    plancode: data.HsnCode || "",
    unitname: "",
    price: "",
    billtype: "",
    billvalue: "",
    billcycles: "",
    freetrial: data.trialDays || "",
    setupfee: "",
    plantype: "goods",
    sac: data.HsnCode || "",
    taxtype: "taxable",
    nontaxreason: "",
    plandescription: data.desc || "",
...
const CreatePlan = ({ open, setOpen, initialData, selectedProduct }) => {
  ...
  const groupId = selectedProduct?.product?.groupId || selectedProduct?.groupId || null;
  ...
  const onSubmit = async (data) => {
    try {
      const payload = {
        productId: productId,
        planName: data.planname,
        desc: data.plandescription || null,
        groupId: groupId,
        hasTrialPlan: Boolean(data.freetrial && Number(data.freetrial) > 0),
        trialDays: data.freetrial ? Number(data.freetrial) : 0,
        billingEvery: {
          value: Number(data.billvalue) || 1,
          unit: data.billtype || "month",
        },
        unitName: data.unitname || null,
        billingCycle: data.billcycles === "autorenewal" ? "Auto" : "Expires",
        taxType: data.taxtype || null,
        features: Array.isArray(data.planfeature)
          ? data.planfeature.map((f) => ({
            name: f.name,
            tooltip: f.tooltipname,
            addNewtag: f.addNewtag,
          }))
          : [],
      };

      if (initialData?.id) {
        await updatePlanMutation.mutateAsync({ id: initialData.id, ...payload });
      } else {
  ...
  return (
    <Box>
      <CommonDrawer
        open={open}
        onClose={handleClose}
        title={initialData ? "Edit Plan" : "Add Plan"}
