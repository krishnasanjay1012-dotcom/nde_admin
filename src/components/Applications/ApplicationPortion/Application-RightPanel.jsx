...
import PlanPriceDrawer from "./PlanPriceDrawer";
...
const PlansPage = ({
  ...
  const handleOpenPriceDrawer = (plan) => {
  ...
  return (
    ...
    >
      ...
      >
        <Typography sx={{ fontSize: "20px", color: "#000", fontWeight: 700, p: 2 }}>
          {selectedProduct?.product?.productName
            ? selectedProduct.product.productName.charAt(0).toUpperCase() +
              selectedProduct.product.productName.slice(1)
            : "Select a product"}
        </Typography>
        ...
        <Box sx={{ display: "flex", gap: "10px" }}>
          <CommonButton
            label={"Edit App"}
            variant="outlined"
            onClick={() => handleEditApplication(selectedProduct?.product)}
            startIcon={false}
            sx={{
          ...
          <IconButton
            ...
            sx={{
              ...
              borderRadius: 2,
              height: 41,
              color: isBulkMode ? "#B0B0B0" : "#919191",
              cursor: isBulkMode ? "not-allowed" : "pointer",
            }}
          >
      ...
      >
        <Box sx={{ backgroundColor: "#F1F2FD", display: "flex", width: "100%", padding: 2 }}>
        ...
        <Box sx={{ display: "flex", flexDirection: "column", width: "100%", padding: 2 }}>
          <Box sx={{ mb: "15px" }}>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr style={{ backgroundColor: "#F1F2FD", border: "1px solid #ddd" }}>
                ...
                {isLoading ? (
                    ...
                    <tr key={plan._id}>
                      ...
                      <td style={{ padding: "15px", verticalAlign: "top", position: "relative" }}>
                        <div style={{ display: "flex", alignItems: "flex-start", gap: "20px" }}>
                          <img
                            src={selectedProduct?.product?.productLogo || "https://via.placeholder.com/60"}
                            alt={plan.planName}
                            style={{
                              width: "60px",
                              height: "60px",
                            ...
                            onClick={() =>
                              handleImageClick(
                                selectedProduct?.product?.productLogo || "https://via.placeholder.com/600"
                              )
                            }
                          />
                          <div style={{ flex: 1, paddingBottom: "10px" }}>
                            ...
                            <Typography variant="subtitle2" sx={{ color: "text.secondary" }}>
                              Plan Code: {plan.HsnCode || plan._id}
                            </Typography>
                            <Typography variant="body1">{plan.trialDays} day(s) free trial</Typography>
                            <Typography variant="subtitle2" color="primary" sx={{ mt: 1 }}>
                              <span style={{ cursor: "pointer" }} onClick={() => handleEdit(plan)}>
                                Edit
                              </span>{" "}
                              |{" "}
                              <span
                                style={{ cursor: "pointer" }}
                                onClick={() => handleOpenPriceDrawer(plan)}
                              >
                                Plan Price
                              </span>{" "}
                              {/* | <span style={{ cursor: "pointer" }} onClick={handleClone}>Clone</span>{" "} */}
                              | <span style={{ cursor: "pointer" }} onClick={() => handleDelete(plan)}>Delete</span>{" "}
                      ...
                      >
                        ...
                        <Typography variant="body2" sx={{ color: "text.secondary", mt: 0.5 }}>
                          Rs.{plan.setupFee || 0} for initial setup
                        </Typography>
                        <div
                          style={{
                            position: "absolute",
                  ...
                  <tr>
                    <td colSpan={2} style={{ textAlign: "center" }}>
                      <Box
                        sx={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 2, mt: 10 }}
                      >
                        <InfoOutlineRounded  sx={{ fontSize: 50,color:'primary.main' }} />
                        <Typography variant="h6">No plans found for this application.</Typography>
                        <Typography variant="body2">
                          Click on "Add Plan" to create a new plan for this product.
                        </Typography>
                      </Box>
                    </td>
                  </tr>
      ...
      <PlanPriceDrawer
        ...
        handleClose={handleClosePriceDrawer}
