...
import {
  useBulkDeletePaymentMade,
  usePaymentMadeCustomView,
...
const paymentMenuItems = [
  ...
  },
  {
    ...
    children: [{ label: "Import Payments" }],
  },
  {
    label: "Export",
...
const PaymentMadeList = () => {
  ...
  const { data: filterPaymentMade } = useCustomerFilterOptions("payment_made");
  ...
  const { data: filterFields } = useFilterFields("payment_made");
  const fields = filterFields?.data || [];
  const rawViewResponse = filterPaymentMade?.entity;
  ...
  const { data, isLoading } = usePaymentMadeCustomView({
    page,
    limit,
    filter: filterType,
  ...
  const deletePaymentMutation = useDeletePaymentMade();
  const bulkDeleteMutation = useBulkDeletePaymentMade();
  ...
  const confirmDelete = () => {
    if (deleteTarget) {
      deletePaymentMutation.mutate(deleteTarget._id, {
        onSuccess: () => {
  ...
  const handlePaymentOptionChange = (view) => {
    if (!view) return;
  ...
  const handleEditFilter = (item) => {
    navigate(`/payments/edit/custom-view/${item?.id}`);
  };

  const handleDeleteFilter = (item) => {
    removeView(
      { module: "payment_made", viewID: item.id },
    );
  };
    ...
    const mappedColumns = headers
      ...
      .map((col) => ({
        ...
        cell: ({ row }) => {
          ...
          // No special concatenation – display raw value
          return value !== undefined && value !== null ? value : "-";
        },
      }));

    return [selectionColumn, ...mappedColumns];
  }, [headers, payments, selectedIds]);
      ...
      <Box display="flex" width="100%" sx={{ height: "calc(100vh - 76px)" }}>
        ...
        <Box flex={1} overflow="hidden">
          ...
          {selectedIds?.length > 0 ? (
            ...
            >
              ...
              <Box display="flex" alignItems="center" gap={1}>
                <CommonButton onClick={handleNew} label="New Payment" />
                <MoreActionsMenu
                  ...
                  onChange={(data) => {
                    ...
                    if (data.type === "child" && data.parent === "Import" && data.label === "Import Payments") {
                      navigate('import');
                    }
                  }}
                />
          ...
          <CommonDeleteModal
            ...
            itemType={
              deleteTarget
                ? `Payment ${deleteTarget.reference || deleteTarget._id}`
                : "Payment"
            }
            title="Payment"
          />
