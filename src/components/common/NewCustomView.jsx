...
import { useAvailableFields, useCreateCustomerCustomView, useFilterFields } from "../../hooks/Custom-view/custom-view-hooks";
...
// Converts react-querybuilder query to MongoDB query
...
const NewCustomView = () => {
    ...
    const { mutate: createView } = useCreateCustomerCustomView();
    ...
    const onSubmit = (data) => {
        ...
        createView({ module: moduleName, data: payload }, {
            onSuccess: () => {
                navigate(-1);
            },
            onError: (err) => {
    ...
    useEffect(() => {
        if (availableColumns?.length) {
            const mandatoryColumns = availableColumns.filter(
                (col) => col.required
            );
            setValue("selectedColumns", mandatoryColumns);
        }
    }, [availableColumns, setValue]);
    ...
    return (
        <Box sx={{ display: "flex", flexDirection: "column", height: "100%" }}>
            ...
            >
                <Typography variant="h4">New Custom View</Typography>
                <CloseRoundedIcon
            ...
            <form onSubmit={handleSubmit(onSubmit)} style={{ height: "100%", display: "flex", flexDirection: "column" }}>
                ...
                >
                    <CommonButton type="submit" label="Save" disabled={!isDirty} startIcon />
